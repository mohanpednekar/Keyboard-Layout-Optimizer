...
def print_final_results(results):
	...
	with open('results.txt', 'a+') as file:
		...
		for key, value in results.items():
			...
			ax.plot(x, c, marker='o')
			ax.plot(x, s, marker='o')
			ax.plot(x, t, marker='o')
...
def testLayout(layout):
	x = layout.calc_old_distance('respective', 'resistance')
	y = layout.calc_new_distance('respective', 'resistance')
	z = layout.calc_ideal_distance('respective', 'resistance')
	print()
	print(str(x) + ' ' + str(y) + ' ' + str(z))

	word = 'respective'
	bs = layout.bSpline[word]
	duration = calc_bspline_duration_for(bs) * correction_factor(word)
	z = speed_for(duration)
	y = layout.calc_speed_using_nGrams(word)
	x = layout.calc_word_speed_for(word)
